...
class CollaborativeRecommender:
    ...
    def build_user_item_matrix(self):
        ...
        interactions = self.db.execute_query(query)
    ...
    def recommend_posts_collaborative(self, user_id: int, n_recommendations: int = 10) -> List[Dict]:
        ...
        user_posts_query = "SELECT DISTINCT post_id FROM interactions WHERE user_id = ?"
        user_posts = set([row[0] for row in self.db.execute_query(user_posts_query, (user_id,))])
        ...
        WHERE i.user_id IN ({})
        GROUP BY i.post_id, p.caption, l.name, p.user_id, u.username
    ...
    def recommend_users_to_follow(self, user_id: int, n_recommendations: int = 10) -> List[Dict]:
        ...
        SELECT 
            ...
            (SELECT COUNT(*) FROM interactions WHERE user_id = f.following_id) as interaction_count
        FROM follows f
        JOIN users u ON f.following_id = u.user_id
        WHERE f.follower_id IN ({})
        GROUP BY f.following_id, u.username, u.full_name, u.bio, u.location, u.travel_style
    ...
    def get_user_travel_stats(self, user_id: int) -> Dict:
        ...
        SELECT 
            ...
            COUNT(DISTINCT i.interaction_id) as interactions_given,
            COUNT(DISTINCT f1.following_id) as following_count,
            COUNT(DISTINCT f2.follower_id) as followers_count,
            COUNT(DISTINCT l.location_id) as locations_visited,
