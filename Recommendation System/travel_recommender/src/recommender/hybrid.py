...
class HybridRecommender:
    ...
    def _get_popular_posts(self, user_id: int, n_posts: int) -> List[Dict]:
        ...
        popular_posts_query = """
        SELECT p.post_id, p.caption, p.user_id, u.username, l.name as location,
        ...
        WHERE p.post_id NOT IN (
            SELECT DISTINCT post_id FROM interactions WHERE user_id = ?
        )
        GROUP BY p.post_id, p.caption, p.user_id, u.username, l.name
        ORDER BY interaction_count DESC
        LIMIT ?
        """
        
        popular_posts = self.db.execute_query(popular_posts_query, (user_id, n_posts))
