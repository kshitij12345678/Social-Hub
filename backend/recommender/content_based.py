...
class ContentBasedRecommender:
    ...
    def build_content_features(self):
        ...
        # Updated for current backend database schema (no post_tags table, no post_type)
        posts_query = """
        SELECT 
            p.id,
            p.caption,
    ...
    def recommend_posts_content_based(self, user_id: int, n_recommendations: int = 10) -> List[Dict]:
        ...
        # Get posts the user has interacted with from all interaction tables
        user_interactions_query = """
        SELECT DISTINCT interactions.post_id, interactions.interaction_type,
               p.caption, l.name as location_name, '' as tag_name
        FROM (
        ...
        user_interactions = self.db.execute_query(user_interactions_query, (user_id, user_id, user_id))
    ...
    def recommend_destinations_content_based(self, user_id: int, n_recommendations: int = 10) -> List[Dict]:
        ...
        all_locations_query = """
        SELECT l.id, l.name, l.country, l.continent, l.category,
               l.latitude, l.longitude,
               COUNT(p.id) as post_count,
               AVG(COALESCE(p.likes_count + p.comments_count + p.shares_count, 0)) as avg_popularity
        FROM locations l
        LEFT JOIN posts p ON l.id = p.location_id
        WHERE l.name NOT IN ({})
        GROUP BY l.id, l.name, l.country, l.continent, l.category, l.latitude, l.longitude
        HAVING post_count > 0
    ...
    def _get_post_details(self, post_id: int) -> Dict:
        ...
        post_query = """
        SELECT p.id, p.caption, p.user_id, p.post_type,
        ...
        WHERE p.id = ?
        GROUP BY p.id, p.caption, p.user_id, p.post_type, u.full_name, u.full_name, l.name, l.country
        """
    ...
    def _get_popular_posts(self, n_posts: int) -> List[Dict]:
        ...
        popular_posts_query = """
        SELECT p.id, (p.likes_count + p.comments_count + p.shares_count) as interaction_count
        FROM posts p
        GROUP BY p.id
    ...
    def _get_popular_destinations(self, n_destinations: int) -> List[Dict]:
        ...
        SELECT l.id, l.name, l.country, l.continent, l.category,
               COUNT(p.id) as post_count,
               SUM(p.likes_count + p.comments_count + p.shares_count) as total_interactions
        FROM locations l
        LEFT JOIN posts p ON l.id = p.location_id
        GROUP BY l.id, l.name, l.country, l.continent, l.category
